...
local function AuctionItem(props)
	if os.time() >= props.value['EndTime'] then
		return
	end
	return e("Frame", {
		...
		Icon = e('ImageLabel', {
			...
			e('TextLabel', {
				...
				Text = props.value['EndTime'] and ('Ends in ' .. Utils.timeLeftString(props.value['EndTime'])) or ' ',
...
AuctionItem = RoactRodux.connect(
    function(state)
        return state
    end,
    function(dispatch)
